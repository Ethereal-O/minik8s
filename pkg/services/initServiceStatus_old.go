...
func InitRuntimeService_old(runtimeService *object.RuntimeService) {
	selectPods_old(runtimeService)
	startPoll_old(runtimeService)
}

func startPoll_old(runtimeService *object.RuntimeService) {
	runtimeService.Timer = *time.NewTicker(CHECK_PODS_TIME_INTERVAL)
	go pollLoop_old(runtimeService)
}

func pollLoop_old(runtimeService *object.RuntimeService) {
	defer runtimeService.Timer.Stop()
	for {
		select {
		case <-runtimeService.Timer.C:
			if runtimeService.Status == SERVICE_STATUS_RUNNING {
				poll_old(runtimeService)
			} else {
...
func poll_old(runtimeService *object.RuntimeService) {
	runtimeService.Lock.Lock()
	defer runtimeService.Lock.Unlock()
	selectPods_old(runtimeService)
}

func selectPods_old(runtimeService *object.RuntimeService) {
